object KotlinConversion {
  def convert(line: String): Option[String] = {
    if (isOnlyAClosingBrace(line))
      ...
      else if ( isAFunctionSignature(line) || isTheEndOfAFunctionSignature(line))
  ...
  def isTheEndOfAFunctionSignature(line: String): Boolean =
    (line.trim.startsWith("):") || line.trim.startsWith(") :")) && line.trim.endsWith("{") && !line.contains("=") &&
      !line.contains("{{")
  ...
  def isAnIf(line:String): Boolean =
    line.contains("if") && line.contains("(") && line.contains(")") && line.trim.endsWith("{")
  ...
  def isTheBeginningOfAClass(line: String): Boolean =
    (line.trim.startsWith("class")
      ...
      || line.trim.startsWith("open class")
      ) && line.trim.endsWith("{")
