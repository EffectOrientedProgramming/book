...
object MicroControllerExample extends zio.App {
  ...
  def run(args: List[String]) = //Use App's run function
    val logic =
      for
        arduino <- Ref.make(Arduino(pin1 = DigitalPin(false)))
        inSeconds <- currentTime(TimeUnit.SECONDS)
        _ <- putStrLn("startTime: " + inSeconds)
        _ <- loopLogic(inSeconds, arduino)
          .repeat(Schedule.recurs(60) && Schedule.spaced(100.milliseconds))
      yield ()
    logic.exitCode
}
