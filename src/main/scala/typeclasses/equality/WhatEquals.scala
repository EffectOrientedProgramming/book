...
/** We are showing the ability to add behavior to classes we don't control.
  * Typeclasses: the generic form of extension methods?
  */
trait WhatEquals[T]:
...
extension [T: WhatEquals](s: String)
  def eq(t: T): Boolean = summon[WhatEquals[T]].strEq(s, t)
