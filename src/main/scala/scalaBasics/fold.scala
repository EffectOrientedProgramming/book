...
object fold {
  ...
  //foldRight() folds FROM the right.
    ...
    val leftFolded = letterList.foldLeft("")( (z,i) =>
        println(s"z = $z")
        println(s"i = $i \n")
        s"$z${i.toString} "
      )
    ...
    val rightFolded = letterList.foldRight("")( (i,z) =>
      println(s"z = $z")
      println(s"i = $i  \n")
      s"$z${i.toString}"
    )
  ...
  // to find the max value in a list.
    ...
    val maxOfList = numList.foldLeft(numList(0))((z, i) =>
      println(s"z = $z")
      println(s"i = $i \n")
      if (i > z)
        i
  ...
  @main def foldEx4 =
    ...
    val strVersion = numList.foldLeft("":String)((z:String, i:Int) =>
      s"$z ${i.toString}"
    )
  ...
  val strList = numList.foldLeft(List.empty[String])( ( z:List[String], i:Int) =>
    i.toString :: z
  )
