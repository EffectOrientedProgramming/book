...
object Fail {
  ...
  @main def MainFail() =
    ...
    val bar: foo2 = foo2()
    val fail3: IO[foo2, Nothing] = ZIO.fail(
      bar
  ...
  // Here we use the different ZIO-based functions to string together a coherent piece of logic.
  val getCreditScore: ZIO[
    zio.console.Console,
    IOException,
    Int
  ] =
    getCreditScoreFromAgency1(false, true)
      .catchAll { case failureReason =>
        for
          _ <- console.putStrLn(
            "First agency failure: " + failureReason
          )
          creditScore <-
