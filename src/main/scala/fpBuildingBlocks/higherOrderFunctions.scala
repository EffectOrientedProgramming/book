...
object higherOrderFunctions {
  ...
  def add(x:Int, y:Int):Int = //[f: (Int, Int) => Int]
    val added = x + y
    println(s"$x + $y = $added!!!")
    added

  def sub(x:Int, y:Int):Int = //[f: (Int, Int) => Int]
    val subtracted = x - y
  ...
  def higherOrder(f: (Int,Int) => Int, x:Int, y:Int) =
    val mathed = f(x,y)
    println(s"I was given a function, Int $x and Int $y. \nThe output is $mathed")
    mathed



  @main def higherOrders =
    val add3n2 = higherOrder(add,3,2)  //Here, we are passing in a function as a parameter
    println("\n")
