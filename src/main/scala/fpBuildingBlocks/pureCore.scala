...
object pureCore {
  //Here, we will be using Options as a way to move effects out of a
  //pure functional core, and into an effectful outside.
  ...
  def transaction(cashPayment:Double, price:Double):Double =
    errorAtNPerc(50)//There will be a 50% chance of random failure to model an effect
    BigDecimal(cashPayment - price).setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble
...
/*
    ...
    //Pure Form
  def transaction2(cashPayment:Double, price:Double):Option[Double] =
      errorAtNPerc(50)//There will be a 50% chance of random failure to model an effect
      BigDecimal(cashPayment - price).setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble
