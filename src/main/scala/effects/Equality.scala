  ...
  val myAppLogic: ZIO[
    ...
    for {
      ...
      _ <- putStrLn(s"""
                       ...
                       | res5: ${res5}
                       | """.stripMargin)
      //Test if the flat mapped ZIO are equivelant:
      _ <- putStrLn(
        (res1 == res2 && res2 == res3 && res3 == res4 && res4 == res5).toString
      )
      //Test if the differently aliased ZIO are considered equivelant:
      _ <- putStrLn(
        (suc1 == suc2 && suc2 == suc3 && suc3 == suc4 && suc4 == suc5).toString
      )
      //Test if identically defined ZIO are considered equivelant:
      _ <- putStrLn((suc4 == suc4d).toString)
    } yield ()

  //Until the ZIO are run, they cannot be considered equivelant to other ZIO. Becuase they represent a certain level of
  //uncertainty, their exact value cannot be determined.
