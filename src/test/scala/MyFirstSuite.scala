...
//  trait BigEnvironmentTrait extends zio.console.Console with zio.clock.Clock
  ...
  currentTime(TimeUnit.MILLISECONDS)
    .flatMap(currentTime =>
      console.putStrLn("Hello, World! Time: " + currentTime)
    )
    .flatMap(_ => ZIO.fail(NewError()))
    .map(_ => ())

  def parameterizedHello(
      name: String
  ): ZIO[zio.console.Console with zio.clock.Clock, IOException, String] =
    for {
      currentTime <- currentTime(TimeUnit.MILLISECONDS)
      _ <- console.putStrLn(s"Hello, $name! Time: " + currentTime)
    } yield (name)

//  val getName: ZIO[zio.console.Console, IOException, Unit] =
//    console.getS("Hello, World!")

object BadConsole:
  val singleFailure = new IOException(
    "Bad stream"
  )
  ...
  val live: Service = new Service:
    ...
    def putStrLn(line: String): IO[IOException, Unit] = ZIO.fail(
      singleFailure
    )
    val getStrLn: IO[IOException, String] = ZIO.succeed("hardcoded string")
