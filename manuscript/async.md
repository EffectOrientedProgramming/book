...
### experiments/src/main/scala/async/AsyncOrSyncDontMatter.scala
```scala
package async
...
### experiments/src/main/scala/async/ListMap.scala
```scala
package async
...
object ListMap extends ZIOAppDefault:
  ...
  override def run =
    ...
    /* numbers.map: i => // some async thing i */

    val mapped =
      ZIO.foreach(numbers): i =>
        defer:
          ZIO.sleep(1.second).run
          i * 2
