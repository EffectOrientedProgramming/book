...
def checkAnswerTry( answer: Int, guess: String ): String =
  try
    if answer == guess.toInt then "You got it!"
    else s"BZZ Wrong!! Answer was $answer"
  catch
    case _ => "User did not provide an integer. Invalid input: " + guess

def checkAnswerZ1( answer: Int, guess: String ): ZIO[Any, Nothing, String] =
  ZIO.attempt(guess.toInt).map( i =>
    if answer == i then "You got it!"
    else s"BZZ Wrong!! Answer was $answer"
  ).catchAll(_ => ZIO.succeed("Invalid input: " + guess))

def checkAnswerZ2( answer: Int, guess: String ): ZIO[Any, Nothing, String] =
  ZIO.attempt(guess.toInt).map( i =>
    if answer == i then "You got it!"
    else s"BZZ Wrong!! Answer was $answer"
  ).orElseSucceed("Invalid input: " + guess)

def checkAnswerZ3( answer: Int, guess: String ): ZIO[Any, Nothing, String] =
  ZIO.attempt(guess.toInt)
...
def checkAnswerZ4( answer: Int, guess: String ): ZIO[Any, Nothing, String] =
  ZIO.attempt(guess.toInt)
    .mapBoth(
...
def parse(guess: String) =
  ZIO.attempt(guess.toInt).orElseFail("Invalid input:  " + guess)

def checkAnswerZSplit( answer: Int, guess: String ): ZIO[Any, Nothing, String] =
  parse(guess)
    .map(
      i =>
        if answer == i then "You got it!"
...
object RunEffectfulGuessingGameTestable extends ZIOAppDefault:
  def run =
    effectfulGuessingGame
      .debug("Result")
      .withConsole(FakeConsole.single("3"))
