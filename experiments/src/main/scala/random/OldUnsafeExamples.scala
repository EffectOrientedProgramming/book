...
def rollDiceZ(): ZIO[RandomBoundedInt, Nothing, Int] =
...
object randNumExZ extends ZIOApp:
  ...
  val layer: ZLayer[ZIOAppArgs, Any, RandomBoundedInt] = RandomBoundedInt.live

  val tag: Tag[RandomBoundedInt] = Tag[RandomBoundedInt]
  def run =
    for
      roll1 <- rollDiceZ()
      roll2 <- rollDiceZ()
