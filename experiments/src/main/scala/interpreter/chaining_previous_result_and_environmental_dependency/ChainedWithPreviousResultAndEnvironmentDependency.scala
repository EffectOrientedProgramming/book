...
trait Operation[Dependencies <: Service : ClassTag]:
  val dep: ClassTag[Dependencies] = classTag[Dependencies]

case class Value(value: String) extends Operation[AnyService]
...
case class RandomString() extends Operation[RandomService]
...
def interpretWithEnvironment(program: Seq[Operation[_]], environment: ToyEnvironment[Service]): String =
