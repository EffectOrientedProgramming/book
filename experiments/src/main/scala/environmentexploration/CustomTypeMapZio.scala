...
case class CustomTypeMapZio[ENV, RESULT](
    ...
    // The tuple here is a step towards the full-featured TypeMap that ZIO uses
    def flatMap[ENV2, RESULT2](
        f: RESULT => CustomTypeMapZio[ENV2, RESULT2]
    ): CustomTypeMapZio[(ENV, ENV2), RESULT2] =
        CustomTypeMapZio(
            (env, env2) => f(run(env)).run(env2)
        )
...
def demoTupledEnvironment =
    val squared: CustomTypeMapZio[Int, Unit] = 
        CustomTypeMapZio(
            env => println("Environment integer squared: " + env * env)
        )

    val repeatMessage: CustomTypeMapZio[String, BigResult] = 
        CustomTypeMapZio(
            message => BigResult(s"Environment message: $message")
        )
    
    val composedRes: CustomTypeMapZio[(Int, String), BigResult] =
        squared.flatMap(_ =>  repeatMessage)

    val finalResult = composedRes.unsafeRun((5, "Hello"))
    println(finalResult)
