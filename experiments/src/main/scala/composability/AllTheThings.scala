...
object AllTheThings extends ZIOAppDefault:
  ...
  def errorThing[A](t: Try[A]) =
    ZIO.fromTry(t)

  val resourcefulThing: ZIO[Scope, Nothing, String] =
    val open = defer:
  ...
  override def run =
    defer:
      // todo: useful order, maybe async first or near first?
      // maybe something parallel in here too?
      // maybe add Future or make asyncThing a Future
      val s: String = resourcefulThing.run
      val t: Try[String] = Success(s) // todo: some failable function
      val w: String = ZIO.fromTry(t).run
      val o: Option[Int] = Option.unless(w.isEmpty)(w.length) // todo: some optional function
      val i: Int = ZIO.fromOption(o).debug.run
      asyncThing(i).run
      // todo: some error handling to show that the errors weren't lost along the way
    .catchAll:
        case t: Throwable => ???
        case _: Any => ???
