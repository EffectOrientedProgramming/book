...
import zio.logging.*
import zio.*
import zio.logging.LogFormat.{label, line, quoted, text}
...
object Logging extends ZIOAppDefault {
  ...
  lazy val locationLogger: LogFormat =
     location(new WackyGps) |-| label("message", quoted(line)).highlight
...
enum Continent:
  case NorthAmerica, SouthAmerica, Europe, Asia, Antarctica, Australia, Africa
...
class WackyGps extends Gps:
  def currentLocation(): Continent =
    Continent.values.apply(scala.util.Random.between(0, Continent.values.length))
