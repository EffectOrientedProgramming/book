...
val zioLogic =
    for
        ...
        state1 <- ZIO.serviceWithZIO[XEnvironment]( env => ZIO(env.increment(1)))
        _ <- printLine("state1: " + state1)
        state2 <- ZIO.serviceWithZIO[XEnvironment]( env => ZIO(env.increment(1)))
        _ <- printLine("state2: " + state2)
    yield ()
object RealZIOEnvironmentPassingExplicitlyProvided extends ZIOAppDefault:
    def run = 
        zioLogic.provide(
            Console.live,
