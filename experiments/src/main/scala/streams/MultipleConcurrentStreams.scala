...
object MultipleConcurrentStreams extends ZIOAppDefault:
  val userActions =
...
//      .throttleShape(1, 1.seconds, 2)(_.length)
  ...
  // Note: I tried to bake this into the mapZIO call above, but that resulted in additional printing
  // for every consumer of the stream. Surprising, but I'm sure there's good reasoning behind it.
  val userActionAnnouncements =
    userActions.mapZIO(action => ZIO.debug("Incoming event: " + action))
  ...
  def run =
    ZStream.mergeAllUnbounded()(userActionAnnouncements, marketingActions, auditingReport, writeActionsToFile).runDrain
