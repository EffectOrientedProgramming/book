...
object DeliveryCenter extends ZIOAppDefault:
  sealed trait Truck
  case class TruckInUse(queued: List[Order], fuse: Promise[Nothing, Unit]) extends Truck
  ...
  def handle(order: Order, staged: Ref[Truck]) =
    ...
    for
      ...
      truckZ <- staged.updateAndGet(truck => truck match
        case TruckInUse(queued, fuse) =>
          TruckInUse(queued :+ order, fuse)
        case TruckEmpty() => TruckInUse(List(order), latch)
      )
      truck = truckZ match {
        case t: TruckInUse => t
        case TruckEmpty() => ???
      }
      _ <- ZIO.debug("Queuing order: " + (truck.queued.length) + "/" + truckCapacity)
      _ <-
        if (truck.queued.length == truckCapacity)
            shipIt *>
            truck.fuse.succeed(())
        else
            ZIO.when(truck.queued.length == 1)(
              ZIO.whenZIO(latch.isDone.map(done => !done))(
      ...
      for
        ...
        orderStream =
          ZStream.repeatWithSchedule(
            Order(),
            Schedule.exponential(1.second, factor = 1.8)
          )
        _ <-
