...
// end
//
// /* Extension work great when the typeclass
// * behavior depends on the value of the T. You
// * don't need an extension method if the
// mapping
// * (type class) is not based on a value (i.e.
// * purely the type).
// *
// * Extension methods aren't just for
// ergonomics,
// * they are an explicit design choice that
// pairs
// * with type classes since most (maybe all) of
// * the time, type classes need to use the value
// * of the type they are operating on, to do
// * something useful. */
//
...
//  val bin = t.bin
//
//  extension (s: Seq[T]) def sort = t.bin
// end seqToBin
//
// /* given[T <: Product] : ToBin[T] with val bin
// =
// * ResortBin extension (p: Product) def sort =
// * ??? */
//
// @main
// def run =
...
// * println((paper1, paper2, cardboard1,
// * goldWatch1).sort) */
// // println(sort("asdf")) // does not compile
// due
